...
def download_pdf(link, location, name):
    ...
    try:
        ...
        print('>>>Error 404: cannot be downloaded!/n')
        raise
...
def clean_text(text, replacements = {' ': '_', '/': '_', '.': '', '"': ''}):
    for key, rep in replacements.items():
        text = text.replacements(key, rep)
    return text
...
if __name__ == '__main__':
	...
	while point is not None:
		if point.name:
			if re.search('h[1-2]', point_name):
				if point.name == 'h1':
					h1_directory = os.path.join(h1_directory, clean_text(point.text))
			...
			if point.name = 'p':
				link = point.find('a')
				if link is not None:
					link = clean_pdf_link(link.attr['href'])
					...
					if not ext in forbidden_extensions:
						...
						try:
							...
							if not os.path.exists('/'.join((curent_directory, fullname))):
